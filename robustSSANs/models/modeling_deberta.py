...
from torch._decomp.decompositions import _softmax_backward_data
# from transformers.pytorch_utils import softmax_backward_data
from transformers.file_utils import ModelOutput
from ..utils.doc import add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward
...
@dataclass
class SequenceClassifierOutputForKL(ModelOutput):
    """
...
class DebertaV2ForSequenceClassificationWithKL(DebertaV2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=SequenceClassifierOutputForKL,
        config_class=_CONFIG_FOR_DOC,
    )
    # Copied from transformers.models.deberta.modeling_deberta.DebertaForSequenceClassification.forward with Deberta->DebertaV2
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, SequenceClassifierOutputForKL]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the sequence classification/regression loss. Indices should be in `[0, ...,
            config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss), If
        ...
        loss, kl_loss, logits, outputs = self.deberta_kl(input_ids, attention_mask, token_type_ids, position_ids,
                                                        inputs_embeds,
        ...
        return SequenceClassifierOutputForKL(
            loss=loss, kl_loss=kl_loss, logits=logits, hidden_states=outputs.hidden_states, attentions=outputs.attentions
        )

    def deberta_kl(self, input_ids, attention_mask, token_type_ids, position_ids, inputs_embeds,
                   labels, output_attentions, output_hidden_states, return_dict):
        ...
        for logits in logits_list:
            if labels is not None:
                ...
                if self.num_labels == 1:
                    ...
                    loss_fct = MSELoss()
                    if loss:
                        loss += 0.5 * loss_fct(logits.view(-1), labels.view(-1))
                    else:
                        loss = 0.5 * loss_fct(logits.view(-1), labels.view(-1))
                else:
                    loss_fct = CrossEntropyLoss()
                    if loss:
                        loss += 0.5 * loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
                    else:
                        loss = 0.5 * loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
        ...
        if loss is not None:
            if self.num_labels == 1:
                loss_fct = MSELoss()
                total_kl_loss = loss_fct(logits_list[0].view(-1), logits_list[-1].view(-1))
            else:
                ...
                total_kl_loss = (kl_loss.sum() + reverse_kl_loss.sum()) / 4.0

        return loss, total_kl_loss, logits_list[0], outputs_list[0]
